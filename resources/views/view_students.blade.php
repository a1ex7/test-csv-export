...
<html lang="en">
    ...
    <body>
        ...
        <div class="header">
            <div><img src="{{asset('images/logo_sm.jpg')}}" alt="Logo" title="logo"></div>
            <div  style='margin: 10px;  text-align: left'>
                <input type="button" id="selectAll" value="Select All"/>
                <input type="submit" form="students" value="Export"/>
                <a href="{{route('export.course')}}" class="button">Export Course Attendence</a>
            </div>
        </div>

        <form id="students" method="POST" action="{{route('export')}}">
            {{ csrf_field() }}
    ...
    <script>
        ...
        selectAll.addEventListener('click', function() {
            const studentIds = document.querySelectorAll('[name^=studentIds]');
